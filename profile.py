...
for i in range(2):
  if i == 0:
    node = request.RawPC("ldapserver")    
  else:
    node = request.RawPC("ldapclient")
...
if i == 0:
  ...
  node.addService(pg.Execute(shell="sh", command="sudo chmod 755 /local/repository/setup_sso.sh"))
  node.addService(pg.Execute(shell="sh", command="/local/repository/setup_sso.sh"))
